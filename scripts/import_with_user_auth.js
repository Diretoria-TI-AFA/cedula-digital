...
async function main() {
  console.log('🚀 Iniciando importação com autenticação de usuário regular...');

  // Autenticar com usuário regular (Diretor Fernando)
  try {
    await pb.collection('users').authWithPassword('fernando@fab.mil.br', 'Password123!');
    console.log('✅ Autenticado com sucesso como Diretor Fernando!');
  } catch (err) {
    console.warn('Criando usuário Diretor Fernando...');
    try {
      ...
      await pb.collection('users').authWithPassword('fernando@fab.mil.br', 'Password123!');
      console.log('✅ Usuário Diretor Fernando criado e autenticado!');
    } catch (cErr) {
      console.error('Erro ao autenticar:', cErr?.message || cErr);
    }
  }
  ...
  console.log(`📊 Planilha lida com sucesso! Total no BANCO DE DADOS: ${bdJson.length} cadetes.`);
